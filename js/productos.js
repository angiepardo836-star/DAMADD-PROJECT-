    ...
    try {
        ...
        productos.forEach(p => {
            ...
            tr.innerHTML = `
                ...
                <td>${parseFloat(p.precio_unitario)}</td>
                <td>${p.estado}</td>
                <td>${p.descripcion}</td>
                <td>
                    <div class="acciones">
...
function editarFila(btn) {
    ...
    // Precio
    celdas[8].innerHTML = `<input type="number" min="0" step="0.01" value="${celdas[8].innerText}">`;
    ...
    // Descripción
    celdas[10].innerHTML = `<input type="text" value="${celdas[10].innerText}">`;
...
async function guardarEdicion(btn) { 
    ...
    const descripcion = celdas[10].querySelector("input").value.trim();
